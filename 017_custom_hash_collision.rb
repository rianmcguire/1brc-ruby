...
def process_chunk(string, offset, limit)
  stations = Array.new(HASH_TABLE_SIZE)
  ...
  while offset < limit
    ...
    name_start = offset
    h = 5381
    while (b = string.getbyte(offset)) != SEMI
    ...
    index = (h & (HASH_TABLE_SIZE - 1))
    name_bytesize = name_end - name_start
    while true
      unless (s = stations[index])
        name = string.byteslice(name_start, name_bytesize)
        s = Agg.new(name)
        stations[index] = s
        break
      end

      # Confirm we've got the correct entry
      existing_name = s.name
      if existing_name.bytesize != name_bytesize
        index += 1
        next
      end
      i = 0
      j = name_start
      while i < name_bytesize
        if existing_name.getbyte(i) != string.getbyte(j)
          index += 1
          next
        end
        i += 1
